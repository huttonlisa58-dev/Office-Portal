...
function CheckInOut() {
  ...
  const doPunch = () => finishPunch(null);
...
export default function Topbar({ onMenu }) {
  ...
  // Requests that need someone to act — the app has no @mentions, so an
  // "Approvals" filter is the useful equivalent of a mentions tab here.
  const APPROVAL_TYPES = ['LEAVE_REQUEST', 'ATTENDANCE_REQUEST', 'ENCASHMENT_REQUEST', 'EXPENSE_REQUEST', 'COMPOFF_REQUEST', 'SHIFT_CHANGE_REQUEST', 'REIMBURSEMENT_REQUEST'];
  const nq = notifQ.trim().toLowerCase();
  const shownNotifs = items.filter((n) => {
    if (notifTab === 'UNREAD' && n.isRead) return false;
    if (notifTab === 'APPROVALS' && !APPROVAL_TYPES.includes(n.type)) return false;
    if (!nq) return true;
    return `${n.title || ''} ${n.body || ''}`.toLowerCase().includes(nq);
  });
      ...
      <div className="relative">
        ...
        {notifOpen && (
          <div className="absolute right-0 top-12 z-30 w-[min(20rem,calc(100vw-1rem))] overflow-hidden rounded-xl border bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
            ...
            <div className="flex items-center gap-1 border-b px-2 py-2 dark:border-slate-700">
              {[['ALL', 'All'], ['UNREAD', 'Unread'], ['APPROVALS', 'Approvals']].map(([k, label]) => (
                <button key={k} onClick={() => setNotifTab(k)}
                  className={`rounded-full px-2.5 py-1 text-xs font-medium transition ${notifTab === k ? 'bg-sky-100 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700/50'}`}>
                  {label}{k === 'UNREAD' && unread > 0 ? ` (${unread})` : ''}
                </button>
              ))}
            </div>
            <div className="border-b px-3 py-2 dark:border-slate-700">
              <input value={notifQ} onChange={(e) => setNotifQ(e.target.value)} placeholder="Search notifications…"
                className="w-full rounded-lg border bg-slate-50 px-2.5 py-1.5 text-xs outline-none focus:border-sky-400 dark:border-slate-600 dark:bg-slate-900/50" />
            </div>
            <div className="max-h-80 overflow-y-auto">
              {shownNotifs.length === 0 && <div className="px-4 py-8 text-center text-sm text-slate-400">{items.length ? 'Nothing matches that filter.' : 'Nothing here yet.'}</div>}
              {shownNotifs.map((n) => (
