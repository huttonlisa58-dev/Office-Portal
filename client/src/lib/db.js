...
export const employees = {
  ...
  async getOne(id) {
    ...
    return {
      ...
      fatherName: data.father_name || null, motherName: data.mother_name || null,
      nationality: data.nationality || null, religion: data.religion || null, aadhaar: data.aadhaar || null,
      physicallyChallenged: !!data.physically_challenged,
      probationEndDate: data.probation_end_date || null, confirmationDate: data.confirmation_date || null,
      noticePeriodDays: data.notice_period_days ?? null, exitDate: data.exit_date || null,
      workLocationId: data.work_location_id || null,
      personalEmail: data.personal_email || null, homePhone: data.home_phone || null,
      communicationAddress: data.communication_address || null,
      city: data.city || null, state: data.state || null, country: data.country || null, postalCode: data.postal_code || null,
...
export const leavePolicies = {
  ...
  async upsert(companyId, leaveType, { annualQuota, accrualPerMonth, eligibilityMonths, carryForwardCap, reasonRequiredDays, expiryMode, expiryDay }) {
    ...
    if (expiryMode !== undefined) {
      row.expiry_mode = expiryMode || 'NONE';
      // the DB enforces: FIXED_DATE needs a day, other modes must not carry one
      row.expiry_day = row.expiry_mode === 'FIXED_DATE' ? (expiryDay || null) : null;
    }
...
export const payroll = {
  ...
  async annualSummary(year) {
    ...
    (data || []).forEach((p) => {
      ...
      if (!by[k]) by[k] = { employeeId: k, currency: p.currency || 'INR', months: 0, gross: 0, tds: 0, net: 0, employee: mEmpRef(p.employee), pan: p.employee?.pan || null };
      by[k].months += 1;
      by[k].gross += Number(p.gross || 0);
      by[k].tds += Number(p.tds || p.tax || 0);
      by[k].net += Number(p.net_pay || 0);
    });
